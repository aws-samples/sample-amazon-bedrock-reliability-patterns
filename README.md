...
As enterprises scale their Bedrock applications and require high availability for mission-critical workloads, they often need capacity beyond individual model quotas. This repository provides **patterns** to access expanded capacity pools across models, regions, and providers while maintaining performance and cost efficiency.

These patterns enable immediate capacity scaling through AWS-native features and custom strategies, empowering you to meet growing demand without waiting for quota increases.
...
graph LR
    ...
    Custom --> C2["Cross-Region Fallback"]
    Custom --> C3["Multi-Model Fallback"]
    Custom --> C4["Multi-Provider Single-Region"]
    ...
    class AWS aws
    class Custom custom
    class A1,A2,A3,A4,A5,C1,C2,C3,C4 pattern
```
...
### **Manual Cross-Region Fallback**
- **When**: Models not available in cross-region inference profiles
...
### **Manual Multi-Model Fallback**
- **When**: Need custom routing logic beyond intelligent prompt routing
- **Code Example**: [patterns/custom/08_multi_model_fallback.py](patterns/custom/08_multi_model_fallback.py)
- **Architecture**: [View Diagram](docs/diagram-generators/pattern8_multi_model_fallback_diagram.md)

### **Manual Multi-Provider Single-Region**
- **When**: Need provider diversification within single region
- **Code Example**: [patterns/custom/09_multi_provider_single_region.py](patterns/custom/09_multi_provider_single_region.py)
- **Architecture**: [View Diagram](docs/diagram-generators/pattern9_multi_provider_single_region_diagram.md)
...
6. Models not in AWS-native features → Custom Patterns
...
python patterns/custom/08_multi_model_fallback.py
python patterns/custom/09_multi_provider_single_region.py
```
...
7. **Cross-Region Fallback** - Manual region failover with detailed status tracking
8. **Multi-Model Fallback** - Sequential model attempts with availability checking
9. **Multi-Provider Single-Region** - Provider-level redundancy within one region
